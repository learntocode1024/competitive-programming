...
void solve() {
  ...
  int lt = wrap(n);
  f[0] = 1;
  for(int len=2;len<=(n<<1ll);len<<=1ll){
	long long lim=len<<1ll;
		
		for(long long i=0;i<lim;i++) g[i]=h[i]=P[i]=0,f0[i]=f[i];
		for(long long i=0;i<len/2;i++){
			g[i*2]=f[i];
			if(i*3<len) h[i*3]=f[i];
		}
		dft(f0,lim);
		dft(g,lim);
		dft(h,lim);
		for(long long i=0;i<lim;i++) P[i]=(f0[i]*f0[i]%p+g[i]%p)*3%p;
		idft(P,lim);
		for(long long i=lim-1;i>=1;i--) P[i]=P[i-1];
		P[0]=p-6;
		
		for(long long i=0;i<lim;i++)
			Q[i]=(f0[i]*f0[i]%p*f0[i]%p+g[i]*f0[i]%p*3+2*h[i])%p;
		idft(Q,lim);
		for(long long i=lim-1;i>=1;i--) Q[i]=Q[i-1];
		Q[0]=0;
		for(long long i=0;i<lim;i++) Q[i]=(Q[i]+p-6*f[i]%p)%p;
		Q[0]=(Q[0]+6)%p;
		
		inv(P,lim);
		mul(P,Q,lim);
		for(long long i=0;i<len;i++) f[i]=(f[i]+p-P[i])%p; 
	}
  FOR(i, 0, lt) cout << f[i] << '\n';
}

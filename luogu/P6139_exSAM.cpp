...
void expand(char ch) {
  // multi-string patch
  if (nxt[ch][lst]) {
    int p = lst, q = nxt[ch][p];
    if (len[p] + 1 == len[q]) {
      lst = q; // patch #1: node exist
    } else {
      int mir = ++tot;
      for (int i = 0; i < 26; ++i)
        nxt[i][mir] = nxt[i][q];
      ...
      lst = mir;
    } // patch #2: split node without producing garbage ;)
    return;
  }
  // normal setup
  int cur = ++tot;
...
void solve() {
  ...
  while (n--) {
    sam::init();
    int m = IO::rdstr(s) - s;
    for (int i = 0; i < m; ++i) sam::expand(s[i] - 'a');
  }
  cout << sam::work() << '\n';
}

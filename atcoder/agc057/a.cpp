...
const i64 p[11] = {1,10,100,1000,10000,100000,1000000,10000000,100000000,1000000000,10000000000};
...
inline void sol() {
  ...
  int k = 0;
  while (p[k+1] <= r) ++k;
  if (r >= 2 * p[k] - 1) O(r-p[k]+1);
  else O(r-p[k]+1+p[k]-max(max(l,r/10+1),r-p[k]+1));
}

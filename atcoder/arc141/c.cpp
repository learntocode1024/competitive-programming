/** @file
 * @ingroup


 */
#include <bits/stdc++.h>
using namespace std;
template <typename T> inline void O(const T &x) { cout << x << '\n'; }
template <typename T, typename... W> inline void O(const T &x, const W &...b) {
  cout << x << ' ';
  O(b...);
}
template <typename T> inline void rd(T &x) { cin >> x; }
template <typename T, typename... W> inline void rd(T &x, W &...b) {
  cin >> x;
  rd(b...);
}
#ifndef MISAKA
#define err(...)
#else
#define err(...) fprintf(stderr, __VA_ARGS__)
#endif
typedef long long i64;
typedef unsigned long long u64;
typedef unsigned u32;
typedef long double dbl;
typedef pair<int, int> pii;
typedef uniform_int_distribution<int> r32;
typedef uniform_int_distribution<i64> r64;
mt19937_64 rng(chrono::steady_clock::now().time_since_epoch().count());
#define shuf(L, R) shuffle((L), (R), rng)
#define fi first
#define se second
#define pb push_back
#define eb emplace_back
#define FOR(i, j, k) for (int i = (j); i <= (k); ++i)
#define ROF(i, j, k) for (int i = (k); i >= (j); --i)
template <typename T> inline void ckmin(T &a, const T &b) { a = min(a, b); }
template <typename T> inline void ckmax(T &a, const T &b) { a = max(a, b); }
//#define IOFILE "c"
//#define MULTI
const int N = 4e5+5;
int p[N], q[N];
int n;
char a[N];

set<int> s;

inline void sol() {
  rd(n);
  FOR(i,1,n*2) rd(p[i]);
  FOR(i,1,n*2) rd(q[i]);
  FOR(i,1,n*2) s.insert(i);
  int lst = 0;
  bool ok = 1;
  for (int i = 1; i < n * 2; i += 2) {
    if (p[i] != *s.begin()) {
      if (p[i+1] != *s.begin()) ok = 0;
      a[p[i]] = '(';
      a[p[i+1]] = ')';
      if (lst > p[i]) ok = 0;
      lst = p[i];
      s.erase(p[i]);
      s.erase(p[i+1]);
    } else {
      s.erase(p[i]);
      if (p[i+1] != *s.begin() || p[i+1] != p[i]+1) ok = 0;
      s.erase(p[i+1]);
      if (lst > p[i]) ok = 0;
      lst = p[i];
    }
  }
  FOR(i,1,n*2) s.insert(i);
  lst = n * 2 + 1;
  for (int i = 1; i < n * 2; i += 2) {
    if (q[i] != *s.rbegin()) {
      if (q[i+1] != *s.rbegin()) ok = 0;
      if (a[q[i]] || a[q[i+1]]) ok = 0;
      a[q[i]] = '(';
      a[q[i+1]] = ')';
      if (lst < q[i]) ok = 0;
      lst = q[i];
      s.erase(q[i]);
      s.erase(q[i+1]);
    } else {
      s.erase(q[i]);
      if (q[i+1] != *s.rbegin() || q[i+1] != q[i] - 1) ok = 0;
      s.erase(q[i+1]);
      if (!a[q[i]] || !a[q[i+1]]) ok = 0;
      if (lst < q[i]) ok = 0;
      lst = q[i];
    }
  }
  if (!ok) {
    O(-1);
    return;
  }
  O(a+1);
}

int main() {
#ifndef MISAKA
#ifdef IOFILE
  freopen(IOFILE ".in", "r", stdin);
  freopen(IOFILE ".out", "w", stdout);
#endif
  ios::sync_with_stdio(0);
  cin.tie(0);
#endif
#ifdef MULTI
  int T;
  cin >> T;
  while (T--)
#endif
    sol();
  return 0;
}

